...
prefix=pathlib.Path(tempfile.mkdtemp(prefix='mesh-sculpt-install-'))
try:
 ...
 if not any(pathlib.Path(f).name in ('mesh_sculpt','mesh_sculpt.exe') for f in files): missing.append('mesh_sculpt executable')
