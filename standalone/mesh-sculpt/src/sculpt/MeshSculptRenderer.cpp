...
MeshSculptRenderer::MeshSculptRenderer(const render::Camera* camera)
    : m_impl(std::make_unique<Impl>(camera))
{
}

MeshSculptRenderer::~MeshSculptRenderer() = default;

void MeshSculptRenderer::upload(const SculptMesh& mesh)
{
...
void MeshSculptRenderer::render(const SculptMesh& mesh, const MeshSelection& selection)
{
    if (!m_impl->camera)
        return;
